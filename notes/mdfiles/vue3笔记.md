...
```vue
const count = ref(0); 
<button @click="count++">{{ count }}</button>  不需要附加.value
```

### 基本数据类型

`ref()`可以使用基本数据类型而 `reactive`不能使用基础数据类型，因为`reactive`使用 `Proxy`代理实现的
...
const obj = reactive({});
// 不能用于基本数据类型
const num = reactive(0) // 错误会丢失响应式
```

### ref 解包细节

> 一个 ref 会在作为响应式对象的属性被访问或修改时自动解包，只有当嵌套在一个深层响应式对象内时，才会发生 ref 解包，**只会对顶层属性解包**
...
> 如果直接给`reactive`赋值一个新的引用类型将丢失响应式，而`ref`则是修改`.value`不会丢失响应式
...
> `computed`
>
> 不需要加括号，因为是读取它的 getter，函数是需要加括号执行的
>
> 访问 `computed` 也是需要 `.value` 进行访问
...
console.log(fullName.value); //使用.value访问
```

> 计算属性会对解构**缓存**与 vue2 相同
...
```js
// 直接传递匿名函数
const randomNum = computed(() => Math.floor(Math.random() * 100));
...
const alwaysSmall = computed((previous) => {
  if(count.value <= 3) {
    return "值为:" + count.value;
  }
  return previous; // previous 为上一次值
});
```
...
<p :class="$attrs.class">Hi!</p>  绑定父级传递的class props
...
>
> `inheritAttrs` 是一个组件选项，用来控制组件的属性**是否被继承**到组件的根元素上。默认情况下为 `true`，会自动被应用到子组件模板中的**第一个**根元素上。如果子组件有多个根元素，那么这些属性将**不会应用**到任何根元素上

```html
<Child class="active" title="标题"></Child>
```

```html
<button @click="update">update</button>
<div v-bind="$attrs"> 绑定 所有 未被捕获的props
  {{ msg }}
</div>
```
...
> 使用`inheritAttrs`禁止继承父级传递的`props`

```vue
<script>
  defineOptions({
    inheritAttrs: false, // 设置禁止继承属性
  });
</script>
<template>
  <main>内容</main>  单个根节点标签将不会自动继承
</template>
```

> 未被捕获的`props`指的是未被 `defineProps();`捕获
...
> v-for 默认是高效渲染,添加key使其可以跟踪，例如`v-move` 动画操作
...
> 可多次调用
>
> 在 `Composition API`中没有`created`和`beforecreated`他们都被替代为 `setup()` 方法
>
> 在`Options API`中仍然生效
...
- `.stop`阻止冒泡
- `.prevent`阻止默认事件
- `.self`只有自身触发,不会因子级元素冒泡触发
- `.capture`设置为捕获,会优先触发自身再触发目标元素
- `.once`一次性事件
...
1. **Getter 函数的作用**：getter函数主要用于获取或计算响应式数据的状态。这个函数会在创建时立即执行一次，以获取初始值，getter函数内部的响应式数据发生变化，Vue就会重新执行这个getter函数。
2. **响应式数据变化**：getter函数内部使用的响应式数据当这些响应式数据的值发生变化时，会自动重新执行getter函数。
3. **返回值**：getter必须返回一个值，返回值会传递给`watch`的第二个参数作为参数传递
...
watch(
  // 这是一个 getter 函数，它会在 watch 被创建时立即执行一次
  //函数内部的响应式数据（x.value 和 y.value）发生变化，Vue 的响应式系统就会重新执行这个函数。
  () => x.value + y.value,
  (sum) => {
...
> 直接传入响应式对象的属性值则会无法侦听，因为等于传递一个**静态值**而通过`getter`返回`getter`会去访问响应式对象从而被`Proxy`拦截到
...
3. 使用 getter 函数的是浅层侦听 `watch(()=>obj.count,(n,0)=>{...})`

   仅当返回值被修改时才会触发

4. 加上 `{deep:true}` 可转化为深层侦听`watch(()=>obj.count,(n,0)=>{...},{deep:true})`

5. `deep` 可以使用**数字**表示层数 `{ deep:1 }`
...
>
> `watcheffect` 不接受 deep 等参数，因为它要去访问响应式值，会自动追踪需要的依赖
>
> `watchEffect`就类似于 watch 里的 getter 函数
>
> `watcEffect` 不接受 `immediate`和 `deep`
...
> 也就是说执行`侦听器`逻辑前会先执行清除副作用函数，确保不会旧的无法销毁
...
>
> 他们两个在使用上没有什么区别，就是命名不一样，为了统一语法规范
...
>
> 设置状态后`watchEffect`的初始化(第一次执行)也将是`flush`指定的顺序，例如指定为`post`则第一次执行就可以访问到模板引用`refs`
...
> `ref` 获取标签
>
> vue 中的定义的 ref 变量必须和dom定义的ref值相同
...
// 旧式获取refs
const myRef = ref(null); // 变量名必须和refs名一致，值一般为null其他也可
onMounted(() => {
  console.log(myRef.value);
});
```

> 使用辅助函数 `useTemplateRef()`
...
> 动态的 `:ref` 绑定才能够传入一个函数。当绑定的元素被卸载时，函数也会被调用一次，此时的 `el` 参数会是 `null`。
...
> 在非`<script setup>`组件中，组件内的属性和方法会通过`setup()`暴露出来，父级获取组件后可以直接访问子组件暴露出来的属性
>
> 而`<script setup>`是私有的，父级无法访问子组件内容，需要通过`defineExpose`暴露出来
>
> `Options`选项卡式默认全部暴露，`setup()`可以通过`expose`暴露，如果没有使用`expose`控制暴露则会将`return`得返回值全部暴露
>
> `defineExpose`等同于`setup()`返回值
...
1. **使用 `defineExpose` 暴露属性或方法**： 在 `<script setup>` 中，调用 `defineExpose` 并传入一个对象，对象的键是你想要暴露的属性或方法的名称，值是实际的属性或方法。
   ...
   defineExpose({
     someData, // 键值名字一样时可以简写
     someMethod
   });
   ```

2. **在父组件中访问暴露的属性或方法**： 父组件可以通过模板引用（`ref`）来访问子组件暴露的属性或方法。
...
> `<script setup>`使用`defineProps`捕获 `props`
>
> 不需要导入，因为它是宏函数，可直接使用
...
> 与父级无关联，会丢失关联，即使父级改变了`props `值也不会同步更改
...
> 即使不写`defineSlots`也可以直接在模板中书写`slots`
...
> 编译宏`defineEmits()` 宏**不能**在子函数中使用。它必须直接放置在 `<script setup>` 的顶级作用域下。
...
<script setup>
console.clear();
const emit =  defineEmits(["enlarge-text"])// 定义需要的emit
...
> `v-model`本质上式利用`props`和`event`双向绑定，父级定义`porps`给子级传递内容，`event`监听子级触发的`emit`，`v-mode`会自动在`父级`监听前缀为`update:`的事件，子级则要`emit`前缀为`update:`的事件
>
> 使用`defineModel`可以更简单的修改，`defineModel(名称,配置项)`

```vue
<script setup>
import { ref } from "vue";
const msg = ref("父级内容"); // 需要响绑定的v-model数据
</script>
<template>
...
<script setup>
// 获取父级v-model
const msg = defineModel(); //没有设置参数为默认v-model 默认props为modelValue
// 直接修改父级属性
msg.value = "子级修改内容"
</script>
```
...
1. ‌**默认调用模式**‌
   当直接调用 `defineModel("title")` 时，返回的是标准响应式 ref 对象，其内部通过 Proxy 封装了 `.value` 的读写操作，自动关联 `modelValue` props 和 `update:modelValue` 事件。
2. ‌**数组解构模式**‌
   使用 `const [model, modifiers]` 解构时，Vue 内部实际上返回的是 `[ref, { trim, number... }]` 的元组。这是通过函数重载实现的特殊返回值结构，用于支持表单修饰符的快速访问。

因为`definemodel`是编译时宏，会在编译时修改原有代码

```html
<BarVue ref="bar" :title="msg" v-model:title.trim="content"> </BarVue>
```
...
```js
// 默认调用模式
const model = defineModel("title"); // ref对象
console.log(model.value); 
// 解构赋值调用模式
const [model, modifiers] = defineModel("title");
console.log(model.value); // ref对象
console.log(modifiers); // 修饰符对象
```

关于使用不一样的方式返回一样值，并不是js原生实现功能，是vue编译器作用

#### get set

> 传入`get`和`set`可以处理接受返回值
>
> 只会在组件组进行修改时触发，父级修改不会触发
...
> 在 Vue 3 中，当父组件向子组件传递属性时，如果子组件**只有一个根元素**，那么这些属性（未被子组件的 props 声**捕获**的属性，包括**事件**）会自动被添加到这个**根元素**上
>
...
> 没有参数的`v-bind`会将一个对象的所有属性（包括事件，会自动处理事件）都作为 attribute 应用到目标元素上。
...
> 插槽内容可以访问到父组件的数据作用域，因为插槽内容本身是在父组件模板中定义的，所以可以直接访问父级数据
...
<template>
  ...
  <main>
    <slot></slot> 没有设置 name 默认为default插槽
  </main>
  <footer>
...
> 允许父组件访问子组件中定义的数据，只有当全部时默认插槽时`v-slot`才可以写在组件上，如果存在具名插槽必须写在`<template>`上

```vue
<template>
  <child v-slot="scope">  // scope 用于接受插槽传递过来内容，写在组件上
    {{ scope.msg.text }}
    {{ scope.msg.count }}
  </child>
</template>
...
> 存在具名插槽时`v-slot`必须写在`<template>`上

父级

```vue
<child>
  <template #header="{ msg }"> 写在template上
    {{ msg }}
  </template>
  <template #main="{msg}">
    {{ msg }}
...
> 如果存在默认插槽和具名插槽则需要为默认插槽使用显式的 `<template>` 标签
...
```text
Root（提供 key: 'color', value: 'red'）
└─ ParentA（提供 key: 'color', value: 'blue'）
   └─ Child（注入 key: 'color'）
```

**Child 组件** 通过 `inject('color')` 获取的值是 `'blue'`（来自最近的父组件 `ParentA`），而不是 `'red'`（来自更远的 `Root`）。
...
> 指定是传递一个getter方法以便更多操作，可以延迟求值，而不是立即返回，每次执行 `getter()` 时才计算值（**延迟**）
...
由于`watchEffect` 是因为监控到了 `getter` 函数内部访问的响应式数据（如 `ref` 的 `value`）才会重新执行。`toValue` 用于获取 `getter` 的值，而 `watchEffect` 的重新执行是由其依赖的响应式数据的变化触发的。如果没有跟踪到依赖项 (例如 url 已经是字符串)，则 effect 只会运行一次

### 自定义指令

> 在 `<script setup>` 中，任何以 `v` 开头的**驼峰式命名**的变量都可以被用作一个自定义指令。在上面的例子中，`vFocus` 即可以在模板中以 `v-focus` 的形式使用。
>
> 只有在**只操作DOM元素**时才建议使用自定义指令。
...
不应该在组件上使用自定义指令，因为如果是多节点组件，指令不能通过 `v-bind="$attrs"` 来传递给一个不同的元素。
...
```html
<div class="box" v-mouse-move:left.once="func">box</div>
```

这里的参数就是 **left**，修饰符就是 **once**，value就是 **func**`value`可以是任何合法的 JavaScript 表达式结果。

例如：

```html
<div class="box" v-mouse-move:left.once="1 + 1">box</div>
```

会返回表达式“**运行后**”的结果，也就是 **2**

#### 例子

```html
<div class="box" v-mouse-move="func" v-if="showBox">box</div>
```
...
const vMouseMove = {
  mounted(el, bind) {
    // 判断value为函数;
    if (typeof bind.value !== "function") throw new Error("值必须为函数！");
    // 执行方法
    const handler = (e) => {
      const { left, top } = el.getBoundingClientRect();
      const x = Math.max(0, Math.floor(e.clientX - left));
      const y = Math.max(0, Math.floor(e.clientY - top));
      bind.value({ x, y });
    };

    // 挂载到dom对象上以用于卸载
    el.__mouseMoveHandler__ = handler;
  ...
  unmounted(el) {
    ...
    delete el.__mouseMoveHandler__;
  },
};
...
export default {
  name: "B",
  inheritAttrs: false, // 是否继承父组件的非 props 属性
  components: {}, // 子组件
  directives: {}, // 自定义指令
  props: ["title"], // 定义 props
  emits: ["update:title"], // 定义可发射的事件
  setup(props, context) {
    // context包括attrs emit expose slots
    // 逻辑代码
    // 通过expose暴露，return将不再暴露
    expose({
      count
    });
    // 暴露变量和方法
...
> `setup`语法糖，语法非常简洁甚至不需要显式声明 `setup` 函数、不需要显式返回 `return` 对象，也不需要显式注册组件

```vue
<script setup> // 添加关键字setup
import { ref } from "vue";
import FooVue from "./components/Foo.vue";// 导入组件
const count = ref(88);
// 通过defineExpose只当暴露值
defineExpose({
  index: count,
